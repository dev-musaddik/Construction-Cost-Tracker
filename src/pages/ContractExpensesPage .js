...
const ContractExpensesPage = () => {
  ...
  // ✅ Fetch categories from expenses API
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        const uniqueCategories = [
          ...new Map(
            fetchedExpenses
              .filter((item) => item.category) // Prevent null categories
  ...
  // ✅ Fetch expenses for selected category
  useEffect(() => {
    if (!selectedCategory) return;

    const fetchContractExpenses = async () => {
      setLoading(true);
      try {
        const userString = localStorage.getItem("user");
        const user = JSON.parse(userString);
        ...
        <div className="bg-white shadow-lg rounded-lg p-6 mb-6 border border-gray-300 hover:shadow-xl transition-all duration-300">
          ...
          <div className="w-full bg-gray-300 rounded-full h-2 mt-4">
            <div
              ...
              style={{
                width: `${Math.min(
                  (totalExpense / agreementAmount) * 100,
                  100
                )}%`,
          ...
          <p className="mt-2 text-lg text-gray-700">
            {t("agreementAmount")}: ${agreementAmount} | {t("amount")}: $
            {totalExpense}
          </p>
